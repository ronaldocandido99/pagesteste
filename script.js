...
// Função para criar e adicionar os flashcards ao DOM
function createFlashCards() {
  const grid = document.getElementById('flashcards-grid');

  flashCardsData.forEach((card, index) => {
    // Criar elementos HTML para cada flashcard
    ...
    frontHeading.textContent = card.front;
    frontDiv.appendChild(frontHeading);

    const backDiv = document.createElement('div');
    backDiv.classList.add('card-back');
    const backParagraph = document.createElement('p');
    backParagraph.textContent = card.back;
    backDiv.appendChild(backParagraph);

    innerDiv.appendChild(frontDiv);
    innerDiv.appendChild(backDiv);
    flashcardDiv.appendChild(innerDiv);
    grid.appendChild(flashcardDiv);

    // Adicionar evento de clique para flip
